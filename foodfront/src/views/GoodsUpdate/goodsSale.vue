<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="产品售价" align="center" min-width="100">
        <template slot-scope="{row}">
          <span>{{ row.price }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisibleNew" top="1%" style="height:95%;overflow-y: auto;" @close="sflag = true" @open="deletePicFlag=false">
      <el-form ref="dataForm" :rules="rules" label-width="150px" :model="goodsInfo" style="width: 500px;margin-left: 200px">
        ...
        <el-form-item label="销售地点" prop="location">
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          price: [{ required: true, message: '必须填写产品价格', trigger: 'blur' }],
    ...
    methods: {
            ...
            this.goodsBlock.data().contract.methods.getbnum(tempList.num).call((error,result)=> {
              ...
              this.goodsBlock.data().contract.methods.getProductName(this.cardNum, tempList.num).call((error, result) => {
                ...
                this.goodsBlock.data().contract.methods.getProductChangeTime(this.cardNum, tempList.num).call((error, result) => {
                  ...
                  this.goodsBlock.data().contract.methods.getProductPrice(this.cardNum, tempList.num).call((error, result) => {
                    tempList.price = result;
      ...
      submitData() {
        ...
        this.goodsBlock.data().contract.methods.setSale(this.goodsInfo.num,this.goodsInfo.price,this.goodsInfo.location,this.goodsInfo.principal,this.goodsInfo.note,this.goodsInfo.QualityState).send({from: this.goodsBlock.methods.getAccount()[0],gas:500000}).then(res=>{
