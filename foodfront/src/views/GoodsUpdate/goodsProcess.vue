<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="加工单号" align="center" min-width="100">
        <template slot-scope="{row}">
          <span>{{ row.processNum }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisibleNew" top="1%" style="height:95%;overflow-y: auto;" @close="sflag = true" @open="deletePicFlag=false">
      <el-form ref="dataForm" :rules="rules" label-width="150px" :model="goodsInfo" style="width: 500px;margin-left: 200px">
        ...
        <el-form-item label="产品加工单号" prop="processNum">
          <el-input v-model="goodsInfo.processNum"></el-input>
        </el-form-item>
        <el-form-item label="产品加工内容" prop="processContent">
          <el-input v-model="goodsInfo.processContent"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          processNum: [{ required: true, message: '必须填写加工单号', trigger: 'blur' }],
          processContent: [{ required: true, message: '必须填写加工内容', trigger: 'blur' }],
    ...
    methods: {
            ...
            this.goodsBlock.data().contract.methods.getbnum(tempList.num).call((error,result)=> {
              ...
              this.goodsBlock.data().contract.methods.getProductName(this.cardNum, tempList.num).call((error, result) => {
                ...
                this.goodsBlock.data().contract.methods.getProductChangeTime(this.cardNum, tempList.num).call((error, result) => {
                  ...
                  this.goodsBlock.data().contract.methods.getProductProcessNum(this.cardNum, tempList.num).call((error, result) => {
                    tempList.processNum = result;
      ...
      submitData() {
        ...
        this.goodsBlock.data().contract.methods.setProcess(this.goodsInfo.num,this.goodsInfo.processNum,this.goodsInfo.processContent,this.goodsInfo.location,this.goodsInfo.principal,this.goodsInfo.note,this.goodsInfo.QualityState).send({from: this.goodsBlock.methods.getAccount()[0],gas:500000}).then(res=>{
