<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="原产地" align="center" min-width="100">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisibleNew" top="1%" style="height:95%;overflow-y: auto;" @close="sflag = true" @open="deletePicFlag=false">
      <el-form ref="dataForm" :rules="rules" label-width="100px" :model="goodsInfo" style="width: 500px;margin-left: 200px">
        ...
        <el-form-item label="产品类型" prop="goodsType">
          <el-input v-model="goodsInfo.goodsType"></el-input>
        </el-form-item>
        ...
        <el-form-item label="原产地" prop="location">
...
<script>
  ...
  import {addUserGoods, addDangerUser, getUserGoods} from "@/api/user";
  import Qs from "qs";
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          id: [{ required: true, message: '必须填写商品编号', trigger: 'blur' }],
    ...
    methods: {
              ...
              this.goodsBlock.data().contract.methods.getbnum(tempList.num).call((error,result)=> {
                ...
                this.goodsBlock.data().contract.methods.getProductName(this.cardNum, tempList.num).call((error, result) => {
                  ...
                  this.goodsBlock.data().contract.methods.getProductChangeTime(this.cardNum, tempList.num).call((error, result) => {
                    ...
                    this.goodsBlock.data().contract.methods.getProductGoodsType(this.cardNum, tempList.num).call((error, result) => {
                      tempList.goodsType = result;
      ...
      submitData() {
        ...
        this.goodsBlock.data().contract.methods.setFarm(this.goodsInfo.id,this.goodsInfo.name,this.goodsInfo.goodsType,this.goodsInfo.note,this.goodsInfo.location,this.goodsInfo.principal,this.goodsInfo.QualityState).send({from: this.goodsBlock.methods.getAccount()[0],gas:500000}).then(res=>{
                                ...
                                self.goodsBlock.data().contract.methods.getProductUserType(i,self.goodsInfo.num).call(function (error,result) {
                                  ...
                                  self.goodsBlock.data().contract.methods.getProductPrincipal(i,self.goodsInfo.num).call(function (error,result) {
                                    ...
                                    self.goodsBlock.data().contract.methods.getProductQualityState(i,self.goodsInfo.num).call(function (error,result) {
                                      ...
                                      for(let m of self.$root.dangerArea){
                                        if(gList.location === m){
                                          self.$root.dangerGoods.push(self.goodsInfo.num)
                                          break
                                        }
                                      }
                                      if (i == self.cardNum)
